...
APP_NAME = 'mp4-video-dark'

if len(sys.argv) < 2:
    print 'Creates a video from MP4 file and adds some brightness.\n Usage:', APP_NAME, '<MP4>'
    sys.exit()
...
RESULT_FILE = 'out.mp4'
FADE_FRAMES = 50
VIDEO_QUALITY = 21 #22-25

gamma      = '1.0'  # <0.1-10> default: 1.0
contrast   = '1.2'  # <-2-2> default: 1.0
brightness = '0.2'  # <-1-1> default: 0.0
saturation = '1.0'  # <0-3> default: 1.0
...
cmd = 'ffmpeg -i concat:"' + outfile1 + '|' + outfile2 + '" -c:v libx264 -preset slow -crf ' + str(VIDEO_QUALITY) + ' -vf "mp=eq2=' + gamma + ':' + contrast + ':' + brightness + ':' + saturation + '" -c:a aac -strict -2 ' + RESULT_FILE
log.info(cmd)
os.system(cmd)
